...
pytestmark = pytest.mark.skipif(not os.environ.get("GEMINI_API_KEY"), reason="Requires GEMINI_API_KEY")
...
def wait_for_server(timeout: int = 90, interval: int = 1) -> bool:
    ...
    while time.time() - start_time < timeout:
        try:
            response = requests.get(AGENT_CARD_URL,connect=3,sock_connect=3,sock_read=5,total=8)
...
def test_agent_card(server_fixture: subprocess.Popen[str]) -> None:
    ...
    response = requests.get(AGENT_CARD_URL,total=20)
...
def test_collect_feedback(server_fixture: subprocess.Popen[str]) -> None:
    ...
    response = requests.post(
        FEEDBACK_URL, json=feedback_data, headers=HEADERS,total=20
    )
    assert response.status_code == 200
